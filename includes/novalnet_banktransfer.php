...
#########################################################
#                                                       #
#  Sofortüberweisung/BANKTRANSFER payment method script #
#  This module is used for real time processing of      #
#  BANKTRANSFER data of customers.                      #
#                                                       #
...
function register_novalnet_banktransfer( $gateways ) {
	$gateways['novalnet_banktransfer'] = array(
		'admin_label' 		=> __( 'Novalnet Instant Bank Transfer', 'edd-novalnet-gateway' ),
		'checkout_label' 	=> __( 'Novalnet Instant Bank Transfer', 'edd-novalnet-gateway' )
	);
	return $gateways;
}
add_filter( 'edd_payment_gateways', 'register_novalnet_banktransfer' );

// register the action to display Test mode description
function novalnet_banktransfer_display_testmode() {
	global $edd_options;
    $test_mode 	 = ( $test_mode = ( edd_is_test_mode() ) ? 1 : ( isset( $edd_options['novalnet_banktransfer_test_mode'] ) ? 1 : 0 ) );
    $information = $edd_options['novalnet_banktransfer_information'] ? trim( $edd_options['novalnet_banktransfer_information'] ) : '';
...
add_action( 'edd_novalnet_banktransfer_cc_form', 'novalnet_banktransfer_display_testmode' );

// register the action to initiate and processes the payment
function novalnet_banktransfer_process_payment( $purchase_data ) {
	global $edd_options;
	$payment_code		= 'novalnet_banktransfer';
...
add_action( 'edd_gateway_novalnet_banktransfer', 'novalnet_banktransfer_process_payment' );

// add the settings of the Novalnet Instant Bank Transfer
function novalnet_banktransfer_add_settings( $settings ) {

	$novalnet_banktransfer_gateway_settings = array(
		array(
			'id' 		=> 'novalnet_banktransfer_settings',
			'name' 		=> '<strong> <font color="red">' . __( 'Novalnet Instant Bank Transfer Settings', 'edd-novalnet-gateway' ) . '</font> </strong>',
			'desc' 		=> __( 'Configure the gateway settings', 'edd-novalnet-gateway' ),
			'type' 		=> 'header'
		),
		array(
			'id' 		=> 'novalnet_banktransfer_test_mode',
			'name' 		=> __( 'Enable Test Mode', 'edd-novalnet-gateway' ),
			'type' 		=> 'checkbox',
			'size' 		=> 'regular'
		),
		array(
			'id' 		=> 'novalnet_banktransfer_merchant_id',
		...
		array(
			'id' 		=> 'novalnet_banktransfer_auth_code',
		...
		array(
			'id' 		=> 'novalnet_banktransfer_product_id',
		...
		array(
			'id' 		=> 'novalnet_banktransfer_tariff_id',
		...
		array(
			'id' 		=> 'novalnet_banktransfer_access_key',
		...
		array(
			'id' 		=> 'novalnet_banktransfer_order_completion_status',
		...
		array(
			'id' 		=> 'novalnet_banktransfer_reference_1',
		...
		array(
			'id' 		=> 'novalnet_banktransfer_reference_2',
	...
	return array_merge( $settings, $novalnet_banktransfer_gateway_settings );
}
add_filter( 'edd_settings_gateways', 'novalnet_banktransfer_add_settings' );